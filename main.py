...
import time
LICNECE = """
Copyright (c) 2021 discord.gg/farinha
"""
...
╚═╝  ╚═╝╚═╝  ╚═══╝ ╚═════╝ ╚═╝  ╚═══╝╚═╝╚═╝  ╚═╝
        ...
        self.slowType("Made by: farinha e gayme", .02)
        time.sleep(1)  # Wait a little more
        # Print the first question
        self.slowType(
            "\nQuantos códigos para gerar e verificar: ", .02, newLine=False)
        ...
        if USE_WEBHOOK:
            ...
            self.slowType(
                "Se você quiser usar um webhook Discord, digite aqui ou pressione enter para ignorar: ", .02, newLine=False)
            ...
            if webhook is not None:
                DiscordWebhook(  # Let the user know it has started logging the ids
                        url=url,
                        content=f"```Comecei a verificar urls\nEnviarei todos os códigos válidos aqui.```"
                    ).execute()
        ...
        for s in c:  # Loop over the amount of codes to check
            ...
            if os.name == "nt":  # If the system is windows
                ctypes.windll.kernel32.SetConsoleTitleW(
                    f"Gerador nitro e verificador - {len(valid)} Valid | {invalid} Inválido - Feito por")  # Change the title
                print("")
